...
unsigned int nStakeMaxAge = 60 * 60 * 24 * 9000; // No max         
...
const string strMessageMagic = "GAY Coin Signed Message:\n";
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "GAY Coin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "GAY - Bitcoin for LGBT Community";
        CTransaction txNew;
        if (!fTestNet)
        {
            txNew.nTime = 1455407990;
        }
        else
        {
            txNew.nTime    = 1455407980; // Without separate txNew.nTime near block.nTime, CheckBlock fails on coinbase timestamp for TestNet created on 14042015
        }
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1455408000;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 4196761;
        if (fTestNet)
        {
            block.nTime    = 0;
            block.nNonce   = 0;
        ...
        // debug print
        assert(block.hashMerkleRoot == uint256("0x1d2c5563988c9657b888cd45c64fc0cf327c01daf6d3355733104e3cc7ef624e"));
        block.print();
